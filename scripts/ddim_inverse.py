...
from ldm.models.diffusion.reverse_ddim import DDIMSampler
...
def main():
    ...
    # sample_path = os.path.join(outpath, "samples-inverse")   
    # os.makedirs(sample_path, exist_ok=True)
    intermediate_path = os.path.join(outpath, "intermediates-inverse")
    ...
    ### Start image that we want to invert
    ### We have to convet this to latent space first
    ### Unforutnately due to clamp it is not a straightforward inversion
    ### Morever, the VQmodel returns a distribution rather than a tensor
    ## We use this instead of samples/0000.png becuase 00050.png is not watermarked

    # img = Image.open(os.path.join(outpath, "intermediates", "x_inter", "00050.png"))
    # print(f"[img.size]: {img.size}")
    # img = np.array(img)
    # print(f"[img.shape]: {img.shape}")
    # img = img / 255.
    # img = rearrange(img, 'c h w -> w c h')
    # img = torch.from_numpy(img)
    # img = img.permute(1, 2, 0)
    # img = img.permute(2, 0, 1)
    # img = img.unsqueeze(0).to(device).float()
    # print(f"[img.shape]: {img.shape}")
    # start_code = model.encode_first_stage(img)
    # print(f"[start_code type]{type(start_code)} ")

    # Or we can cheat and use the latent space directly (skip the encoder step)
    start_code = torch.load(os.path.join(outpath, "intermediates", "x_inter_pickle", "00050.pt"), map_location=device)
    print(f"[start_code]{start_code.shape} ")
    ###
    with torch.no_grad():
            ...
            with model.ema_scope():
                ...
                for n in trange(opt.n_iter, desc="Sampling"):
                    for prompts in tqdm(data, desc="data"):
                        ...
                        for output_variable, o2, o3, o4 in [("x_inter", "x_inter_latents", "x_inter_values", "x_schedule")]:
                            ...
                            x_schedule_path = os.path.join(intermediate_path, o4)
